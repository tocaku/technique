...
const DownloadCard: React.FC<DownloadCardProps> = ({
  ...
  return (
    <MagicCard className="w-full max-w-[90%] lg:max-w-64 flex flex-col bg-white  p-5 rounded-2xl border-2 border-interbrasGray/65">
      <AvatarCircles avatarUrls={avatarUrls} pricipalIcon={pricipalIcon} />
...
export const Downloads: React.FC = () => {
  return (
    <>
      <Helmet>
        <title>Electroshop - Tocaku</title>
        <meta
          name="description"
          content="Загрузите файлы tocaku на этой странице!"
        />
      </Helmet>
      <Modal>
        <DownloadPage />
  ...
  return (
    <div className=" w-full lg:h-20 rounded-xl flex flex-col lg:flex-row gap-2 bg-gray-100 border p-2 ">
      <div className=" flex ">
        <img
          src={imageCard}
          className=" size-[48px] my-auto ml-2 rounded-xl border"
          alt=""
        />
        <div className=" ml-3  my-auto">
          ...
          <div className=" flex gap-2">
            {models.map((model, index) => (
              <p
                key={index}
                className=" text-black/70 bg-black/10 px-2 rounded-lg"
              >
                {model}
              </p>
            ))}
  ...
  const files = [
    {
      ...
      files: [
        () => {
          return (
            <DownloadModalCard
              name={t("downloads.files.scooter105y85.1")}
              models={["10.5", "8.5 pro"]}
              imageCard="/downloads/google-icon.png"
              url={ScooterAndroidUrl}
            />
          );
        },
        () => {
          return (
            <DownloadModalCard
              name={t("downloads.files.scooter105y85.2")}
              models={["10.5", "8.5 pro"]}
              imageCard="/downloads/apple-icon.png"
              url={ScooterIosUrl}
            />
          );
        },
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className=" w-full flex-wrap flex gap-3   lg:justify-start justify-center "
          id="apps"
        >
          ...
          <ModalBody>
            ...
            <ModalFooter className="gap-4 mt-2">
              <ModalTrigger className="px-2 py-1 bg-interbrasGreen-500 text-white  border border-gray-300 rounded-md text-base w-28">
                {t("downloads.close")}
              </ModalTrigger>
            </ModalFooter>
          </ModalBody>
